...
class LocationViewController: UIViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    //MARK: 初始化一些视图
    func initView(){
        let frame = CGRect(x: 0, y: -65, width: width, height: UIScreen.mainScreen().bounds.height-64-25)
        ...
        thirdTableView = UITableView(frame: CGRect(x: 2*width, y: -65, width: width+10, height: UIScreen.mainScreen().bounds.height-64-25), style: UITableViewStyle.Plain)
