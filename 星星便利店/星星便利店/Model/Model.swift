...
class Model: NSObject {
    ...
    func loadDataForNetWork(success: (() -> Void)?){
        ...
        if UserAccountTool.userIsLogin() {
            userID = UserAccountTool.getUserCustNo()! 
            if(userID == nil){
                return
            }
            let appAddress = UserAccountTool.getAppAddressInfo()
            let address = "\(appAddress![0])-\(appAddress![1])-\(appAddress![2])"
                    ...
                    if let jfModellist = showCar["cartList"] as? NSArray {
                        for var i=0 ; i<jfModellist.count ; i++ {
                            if let jfModel = jfModellist[i] as? NSDictionary{
                                ...
                                let itemDistPrice = jfModel["discountPrice"] as? Double
                                JFmodel.itemSalePrice = "\(itemSalePrice!)"
//                                if itemDistPrice != nil
//                                JFmodel.itemDistPrice = "\(itemDistPrice!)"
