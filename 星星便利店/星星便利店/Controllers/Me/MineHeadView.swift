...
class IconView: UIView {
    ...
    func prepareUIForIsLogin(){
        if UserAccountTool.userIsLogin() {
            ...
            let range  = Range(start: phoneNum.text!.startIndex.advancedBy(3), end: phoneNum.text!.endIndex.advancedBy(-4))
            phoneNum.text = "\(phoneNum.text!.stringByReplacingCharactersInRange(range, withString: "****"))"
        }
    }
}