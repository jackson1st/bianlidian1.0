...
class MeViewController: BaseViewController,UINavigationControllerDelegate {
...
extension MeViewController: UITableViewDelegate, UITableViewDataSource {
    func numberOfSectionsInTableView(tableView: UITableView) -> Int {
        if UserAccountTool.getUserRole() != "1" {
            return 2
        }
        return 3
    }
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if indexPath.section == 2{
            ...
            if indexPath.row == 0 {
                ...
                let vc = story.instantiateViewControllerWithIdentifier("UpdateImageViewController")
                navigationController?.pushViewController(vc, animated: true)
            }
            if indexPath.row == 1 {
                let story = UIStoryboard.init(name: "ManagerStoryboard", bundle: nil)
                let vc = story.instantiateViewControllerWithIdentifier("ShopManageViewController")
                navigationController?.pushViewController(vc, animated: true)
            }
        }
