...
class SearcherResultViewController: SearcherViewController {
    ...
    override func viewDidLoad() {
        ...
        showSuperView(true)
        lackOfInformation = LackOfInformationView(frame: CGRectMake(0, 0, AppWidth, AppHeight), imageName: "noorders", title: "暂无此类商品")
...
extension SearcherResultViewController{
    ...
    */
    func loadData(index: Int,count:Int,success:(data:[SearchItemResult]) -> Void){
        ...
        let parameter1 = ["address": address, findByClass ? "classno":"itemname": keyForSearchResult, "pageindex":index,"pagecount":count,"ordercondition": ordercondition!, "orderstyle": orderstyle == true ? "asc" : "desc"] as  [String: AnyObject]
        let parameter2 = ["address": address, findByClass ? "classno":"itemname": keyForSearchResult, "pageindex":index,"pagecount":count] as [String: AnyObject]
        ...
        HTTPManager.POST(ContentType.SearchResultListByItemName, params: self.ordercondition == "none" ? parameter2 : parameter1).responseJSON({ (json) -> Void in
            ...
            if(index <= size){
                self.index = index
                for var x in arr{
                    data.append(SearchItemResult(no: x["itemNo"] as? String,name: x["itemName"] as? String, itemByNum: x["itemBynum1"] as? String, price: x["itemSalePrice"] as? String, brandName: x["brandName"] as? String, eshopIntergral: x["eshopIntegral"] as? Double, url: x["url"] as? String,discountPrice: x["discountPrice"] as? String))
                }
            }
...
extension SearcherResultViewController{
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if(tableView.tag == 101){
            ...
            let vc = mainStoryBoard.instantiateViewControllerWithIdentifier("itemDetail") as! OtherViewController
            vc.address = address
            vc.itemNo = itemno
