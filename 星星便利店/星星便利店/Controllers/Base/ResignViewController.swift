...
extension ResignViewController {
    func sentCodeClick() {
        ...
        let hud = MBProgressHUD.showMessage("请稍等")
        let param: [String : AnyObject] = ["tel" : phoneTextField.text! , "password" : DES3Util.encrypt(psdTextField.text!)]
        HTTPManager.POST(ContentType.ValidateAndSend, params: param).responseJSON({ (json) -> Void in
            ...
            SVProgressTool.showErrorSVProgress("验证码发送失败")
        }
    }
